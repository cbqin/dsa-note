...
class Twitter:
    ...
    def getNewsFeed(self, userId: int) -> List[int]:
        ...
        posts = self.users[userId].tweet[-10:][::-1]
        for followee in self.users[userId].followee:
            if followee in self.users:
                posts_ee = self.users[followee].tweet[-10:][::-1]
                i = 0
                j = 0
                combine = list()
                while i < len(posts) and j < len(posts_ee):
                    if self.tweet_time[posts[i]] > self.tweet_time[posts_ee[j]]:
                        combine.append(posts[i])
                        i += 1
                    else:
    ...
    def unfollow(self, followerId: int, followeeId: int) -> None:
        ...
        if followerId != followeeId:
            if followerId in self.users:
                self.users[followerId].followee.discard(followeeId)
