...
class Solution:
    def findPeak(self, left, right, mountain_arr):
        while left < right:
    ...
    def binarySearchLeft(self, target, left, right, mountain_arr):
        while left < right:
            mid = left+(right-left)//2
    ...
    def binarySearchRight(self, target, left, right, mountain_arr):
        while left < right:
    ...
    def findInMountainArray(self, target: int, mountain_arr: 'MountainArray') -> int:
        ...
        peak = self.findPeak(left, right, mountain_arr)
        left_position = self.binarySearchLeft(target, left, peak, mountain_arr)
        if left_position != -1:
            return left_position
        right_position = self.binarySearchRight(
            target, peak, right, mountain_arr)
